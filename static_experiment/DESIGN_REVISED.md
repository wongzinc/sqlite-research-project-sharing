# Static SQLite Layout 與 Prefetch 實驗規格（修訂版）
...
`cJSON` 必須以現成 library source vendored 於 `tools/vendor/cjson/`，不得自行實作 JSON parser，亦不得要求執行環境預先安裝 system cJSON package。SQLite amalgamation必須vendored於`tools/vendor/sqlite/`；build不得依賴system SQLite development package。
...
Training profile是measurement開始前的必要輸入。任一training run timeout、回傳非零、snapshot失敗或profile聚合失敗時，orchestrator必須中止該次實驗並保留training logs；第19節「繼續其餘cells」只適用於measurement cell階段。
...
- 不接受命令列參數。
- 為可執行的一般檔案。
- 自行執行`sync`，或透過執行環境核准的等效機制完成該動作。
- 使`3`被寫入`/proc/sys/vm/drop_caches`，或透過工作站管理者提供的受控介面完成等效的全系統page-cache清除。
- 成功時exit 0。
- 失敗時以非0結束。

`drop_caches_use_sudo=true`時，harness必須以`sudo -n`執行helper；設定為`false`時必須直接執行helper。Helper stdout與stderr由harness繼承。Helper非0結束時，harness必須以非0結束，cell必須記錄為failed。

實驗不得假設helper固定為專案內`tools/bin/drop_caches.sh`或固定安裝於`/usr/local/sbin/`。在無root權限工作站上，可以使用管理者預先提供的另一支helper；執行帳號本身不需要root權限。該helper仍必須符合本節的無參數、非互動、exit status與cache清除語意。其絕對路徑由`paths.drop_caches_script`指定：需要passwordless `sudo -n`時設定`drop_caches_use_sudo=true`，可直接執行時設定為`false`。
...
Cell unit name必須由cell ID產生並保持唯一。Wrapper必須將stdout、stderr與artifacts直接寫入experiment cell與log目錄。Orchestrator必須使用前景執行的`systemd-run --scope` process exit code作為cell process exit code，不得加入`--wait`。`--expand-environment=no`不得省略；wrapper path與其arguments必須原樣傳入scope，不得由`systemd-run`展開環境變數。
...
`drop_caches_script`是執行環境介面，不要求不同主機使用相同檔名或路徑。移植至工作站時必須在experiment開始前設定為該主機提供的helper絕對路徑，並使`drop_caches_use_sudo`符合實際呼叫方式。
...
- `layouts`至少包含一個layout；`execution.layout_order`必須恰好包含所有layout且不得重複。
...
- `execution.strategy_order`必須恰好包含所有configured strategy名稱且不得重複。
...
- 將目前validation phase可發現的errors輸出至stderr；若必要欄位或型別不足以安全進行後續檢查，可先結束該phase。
- Process exit code非0。
- 不建立experiment目錄。
...
Memory-limited scope可能在kill命令送達前已自行結束並被`--collect`回收；此時`systemctl`可輸出`Unit ... not loaded`。此race不改變cell的timeout判定，也不要求該訊息數量與timeout cell數一一對應。
...
已完成cell只有在identity一致且必要artifact通過下列解析與一致性檢查時才跳過。中斷中的cell從該measurement workload與repetition開頭重跑，不從query或operation level恢復。
...
Run record必須能解析固定summary且其`output`路徑必須指向該cell的operations CSV；operations CSV必須具有固定header且至少包含一筆operation。非baseline的prefetch result必須為可解析JSON，且`cell_id`、`status=completed`與memory condition必須和cell一致。
...
Experiment後處理完成後，`state.json`的`status`記為`completed`，表示排程、彙整、繪圖與報告流程已走完，不代表所有cells成功。`completed`記錄成功cell數；`failed`記錄所有非completed cells總數，使用者必須配合report或cell JSON判讀failed、timeout與invalid分類。
...
Raw CSV不新增合成欄位。`summarize_results.py`必須由每個completed cell的既有raw欄位派生effective first-query指標，寫入summary與comparison CSV，並由report呈現。
...
`all_raw.csv`必須包含實驗內全部cells，每個cell恰好一列，並依第18節定義的cell規格展開順序排列。此順序為可由config、manifest與cell identity重新建構的canonical順序，不得使用filesystem traversal順序、檔案mtime或resume時的實際完成時間排序。此檔案用於跨分類查閱與複製，不得取代各strategy variant目錄中的`raw.csv`。
...
- First-query latency。
- Effective first-query latency（包含prefetch cost）。
...
Effective指標必須逐cell計算：

```text
effective_first_query_latency_us =
    prefetch_elapsed_us + first_query_latency_us

effective_first_query_improvement_percent =
    (baseline_first_query_latency_us - effective_first_query_latency_us)
    / baseline_first_query_latency_us × 100
```

Baseline沒有prefetch cost，其`effective_first_query_latency_us`等於`first_query_latency_us`。Effective improvement必須和相同layout、workload type、memory condition、measurement file與repetition的baseline first-query latency配對後逐sample計算，再聚合其distribution。若prefetch或first-query latency缺失，該cell的effective指標亦缺失。
...
X軸與Y軸均顯示p25–p75 error bars。Y軸使用same-layout paired baseline improvement。同一backend圖必須依memory condition分成並排子圖；子圖共享依所有P25–P75範圍加padding計算的Y軸尺度。X軸在所有P25均大於0時使用log scale。點形區分layout與strategy variant，圖例以`workload:layout:short-strategy`標示點位。
...
Effective first-query latency median
Effective first-query improvement vs same-layout baseline
Average-query latency median
...
Effective first-query latency median
Effective first-query improvement vs shared baseline
Average-query latency median
...
Effective first-query latency median
Effective first-query change vs reference condition
Average-query latency median
...
報告必須列出各cell status的數量。Failed、timeout與invalid cells必須列出cell ID、workload、layout、memory condition、backend、strategy與對應log連結。Baseline的backend必須顯示為`—`。

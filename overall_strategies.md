...
>
> **本檔以下章節描述的策略「機制」皆已正確**（OS primitive 用什麼都沒變），
> 但**結論段引用的歷史 µs 數字**仍是 pre-P0 mixed-pipeline 量出來的——下面
> 用 `> ⚠️` 標出具體位置與建議修法。所有絕對 µs 數字以及策略相對改善 %
> **都需要在 P0 pipeline 下重跑驗證**才能納入論文最終版。
...
> 改善」結論但 baseline 點不同)。
>
> ⚠️ **P0 待修**：上表的 baseline 318 / 463 / 467 µs 是 `posix_fadvise` per-file
> pipeline (P1) 量出的，跟 [CONTRADICTIONS.md](CONTRADICTIONS.md) #1 列出的其他
> 個位數值（73 / 251 / 299.7 / 305 / 321 / 404 / 505 / 634）來自不同 pipeline。
> **建議修法**：以 P0 pipeline（`/usr/local/sbin/drop-caches` 全機 drop +
> harness 內建 `--verify-hotset` 量 `cold_pct`)用 `run_p0.py` 重跑全部
> (workload, layout) × baseline cell（P0 已內建 no-prefetch baseline 臂),
> 取代本表 + [overall_results.md](overall_results.md) 主表的所有 baseline 數字。
...
> ⚠️ **P0 待修（多項待重跑驗證）**：
> 1. 上述 2c 全部 % 數字混用了 P1（layout/access/slru）與 P2（churn）pipeline
>    的 baseline，跨 sub-project 比較性受影響——[CONTRADICTIONS.md](CONTRADICTIONS.md)
>    #2 已抓到 layers_5 on A 在不同表有 5 種值（−54% / −41~42% / −30% / −29% / −47%）。
>    **建議修法**：以 P0 重跑所有 (A,B,C,Z) × (1a,1b,1c) × N∈{0,1,5,10,20,46,92} cell。
> 2. [CONTRADICTIONS.md](CONTRADICTIONS.md) #19 抓到 "plateau vs U 型退化" 在
>    不同段落自相矛盾（「N=5 就 plateau、churn 不改變」vs「載 92 比載 5 差」）——
>    **建議修法**：churn 與非 churn 用同一 P0 pipeline 重跑、文字統一改寫為「N=5 是
>    syscall vs coverage 的 sweet spot；N>5 出現 plateau 或退化視 layout 與 workload 而定」。
> 3. [CONTRADICTIONS.md](CONTRADICTIONS.md) #18 抓到「載太多變慢」歸因 vs
>    「prefetch overhead 可忽略」彼此抵觸——**建議修法**：兩種因果敘述擇一統一
>    （建議：N>5 退化是 async madvise 來不及在 first query 前完成，**不是** syscall
>    overhead，因為 calibration 量出 layers_92 prefetch overhead 也只 ≈14 µs）。

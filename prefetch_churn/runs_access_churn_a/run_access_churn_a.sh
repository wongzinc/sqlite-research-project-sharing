...
#   - 2e_k10_static : access-pattern with static t=0 hot2e_A_orig_K10.csv (expected decay)
#   - 2d_static     : access-pattern interior-only with static hotpages_a.csv  (expected mild decay)
#   - (baseline + layers_92 reuse runs_nsweep_a/n0 and runs_nsweep_a/n92 directly)
#
# K=50 added as supplementary to see if more leaves help static survival.
set -u
...
EOF
  ...
  echo "=== A: ${label} (mode=${mode}, hot=$(basename $hot_csv), cap_leaf=$cap_leaf) ==="
...
echo "ALL DONE (Workload A × churn, static t=0 hotpages)"

#!/bin/bash
# Full N-sweep on churned DB × Workload B (Zipfian), N=0..92 (all 93 values)
...
#!/bin/sh
exec $EVICT $outdir/test_churn.db >&2
EOF
  ...
  python3 sqlite_prefetch_churn_experiment.py \
    ...
    --drop-caches-script "$evict_script" --no-drop-caches-use-sudo \
...
echo "ALL DONE (Workload B full sweep)"

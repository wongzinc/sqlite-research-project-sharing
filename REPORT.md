...
研究（以下數字全部來自 P0 master batch,權威表見 §5 / [overall_results.md](overall_results.md);全 cell `cold_pct`=0）：實驗顯示既有 cache-dump 策略（2f_slru,載入整個 resident working set）雖能把 first-query latency 壓到最低（**−79 ~ −90%**,A/orig 497→107 µs、C/orig 1058→102 µs），但其 **~6–7.5 ms 的 preprocessing 開銷** 反讓 end-to-end cold start **慢上一個數量級**——這個 trade-off 在既有 prefetch 文獻中長期被忽略。相對地,**page-type-aware 的結構式 / access-pattern prefetch（layers_5 / 2e_K10）** 以**極少 syscall** 取得 first-query **−30 ~ −85%**(file-tail uniform workload 上 access-pattern 與盲載全部 interior 相當,皆約 **−47%**);關鍵在於 **end-to-end 的勝負取決於 baseline 夠不夠慢**:慢 workload（C, baseline ~1058 µs）上 2e_K10 達 **e2e −56%（462 µs）**,而快 workload（A, baseline ~497 µs）上 warmer 的 preprocessing 反而蓋過 first-query 的省時。三條 robustness 軸——**50k 寫入 churn**(static t=0 hotset 不衰退)、**cgroup `MemoryMax=20M`** 記憶體壓縮(20M/不限 比值近 1.0)、與多-process **cadence 重暖**——下結論皆穩定。
...
的參照資料庫（102 MB；orig/ta 26,331 page、vacuum 25,613 page）上，P0 重跑的
...
> **以下貢獻數字全部依 P0 master batch 更新(權威表見 §5 / [overall_results.md](overall_results.md))。**

- **(C1) Type-aware layout rewriter**：實作並驗證在 binary 層級重排 SQLite
  file 並修補所有 page-number reference 的可行性與正確性。P0 下結構式
...
- **(C2) Access-pattern frugality**：基於 `mincore()` snapshot 的
  ...
  **−30 ~ −85%**;在 file-tail uniform(B)上與盲載全部 interior 相當(皆約
...
- **(C3) Preprocessing cost-accounting 框架**：據我們所知，首次將 prefetch
  ...
  [Yi+26]）。P0 證實既有 cache-dump 策略(2f_slru)雖把 first-query 壓到最低
  (**−79 ~ −90%**),但其 **~6–7.5 ms preprocessing** 反讓 end-to-end cold
  start **慢一個數量級**——此 trade-off 在既有 prefetch 文獻中長期被忽略。
- **(C4) Robustness 三維驗證**：50k 寫入 churn 後 static t=0 hotset 不衰退
  (C 上 2e_K10 跨 checkpoint 持平 ~99 µs vs baseline ~580);cgroup
  `MemoryMax=20M` 記憶體壓縮下 first-q 幾乎免疫(20M/不限比值近 1.0);多 process
  cadence 重暖下,cadence ≤
...
> `drop-caches` wrapper + harness 內建 `--verify-hotset` 量 `cold_pct`/`delivery_pct`，
> 非外部 residency_checker）即為
...
| **(iii) Cold-start clearing**（P0 第①+②層）| `--cold-advice dontneed` 對 mmap region 依序執行 `madvise(MADV_COLD) → MADV_PAGEOUT → MADV_DONTNEED`；緊接 `--drop-caches-script /usr/local/sbin/drop-caches` 進行全機 `sync; echo 3 > /proc/sys/vm/drop_caches`；接著 **harness 內建** `--verify-hotset` 做一次 `mincore`，emit `verify_cold_pct`（應 ≈0；>1% 的 cell 由 runner 在彙整時剔除，harness 不自動 abort）| §2.2 表 OS page cache 行 |
| **(iv) Post-cold prefetch**（P0 第③層）| `--post-cold-script` 執行統一交付引擎 **`warmer`**（`WARM_METHOD=pread` 或 `fadvise`，hotset 由離線的 `prefetch_layers/access/slru` 預先產生並凍結，見 §3.4.1）；warmer 在 stderr 自報 `warmer_us`；**baseline cell 不帶 post-cold-script（不做任何 prefetch）** | §2.2 P0 banner 第③層 |
| **(v) Delivery residency check** | drop-caches→prefetch 後、首查之前，harness 再做一次 `mincore`（~µs），emit `verify_delivery_pct`＝首查實際看到的命中率（µs 級，async readahead 來不及多載 → 不污染 `fq_async`，修復漏洞 2）| prefetch coverage 驗證 |
| **(v′) CPU 頻率暖機** | `--warm-cpu-ms`（預設 10ms）對 taskset 釘定的核心 busy-spin，把 amd-pstate 拉到滿頻**才**進首查，消除「最快 cell 受 freq ramp 懲罰最重」的偏差；spin 在計時區外 | 量測前 sanity |
...
每個 cell 對**同一 hotset** 跑 **pread（oracle）/ async（hint）雙臂**，外加每 (workload,layout) 一個 **baseline（無 prefetch）cell** 當分母。輸出欄位（`run_p0.py` → `raw_p0.csv` / `summary_p0.csv`）：`verify_cold_pct`、`verify_delivery_pct`、`fq_pread`、`fq_async`、`delivery_pct`、`preproc_us`（= `warmer_us`）、`e2e_us`（= preproc + fq）、`avg_us`、`majflt`、`minflt`，每筆並記 `ra_kb` 與當下 `loadavg/memavail`。完整 schema 見
[IMPLEMENTATION_PIPELINES.md](IMPLEMENTATION_PIPELINES.md) §3。

> **P0 嚴格性聲明**：冷清快取一律 `madvise chain（process-local）+ 全機 drop-caches`，殘留驗證一律走 **harness 內建 `--verify-hotset`**（兩道 `mincore`：cold ① 應 ≈0、delivery ② 即首查命中率），**不**用外部 `residency_checker`——後者的 ~100 ms 間隔會讓 async readahead 多載、污染 `fq_async`（漏洞 2），故只保留作離線/regen 用途。`cold_pct>1%` 的 cell 視為冷清失敗、由彙整剔除。歷史上採用 per-file `posix_fadvise(DONTNEED)` 或 `sudo drop_caches` 量出的數字一律標記為 pre-P0、由 master rerun 取代（詳見 [IMPLEMENTATION_PIPELINES.md](IMPLEMENTATION_PIPELINES.md) §3 與 [CONTRADICTIONS.md](CONTRADICTIONS.md) #24/#27）。
...
其中 P0 的 `prefetch_us` ＝統一交付引擎 **`warmer`** 的 preprocessing wall-clock
（warmer 在 stderr 自報 `warmer_us=...`，pread/async 兩臂同一引擎、只差 sync/async），
與 `first_query_us` 在同一個 P0 cell、同一 rep 內依序量測。
...
wall-clock 跟線上跑時數量級一致；(3) 早期對各 native tool 另做的 offline
calibration（[calibration/prefetch_time_summary.csv](calibration/prefetch_time_summary.csv)）
僅留作 sanity-check，**§5 的 `prefetch_us` 一律取自每個 async cell 的 live
`warmer_us`（per-rep median）**，不再從 native-tool 數字補齊（避免混引擎、重複計入）。
...
**（P0 重跑數據,async 臂,median;權威表見 [overall_results.md「P0 master batch 結果」](overall_results.md)。舊 P1「318→127 −60%」整組數字已由下表取代。）**
...
> **P0 關鍵修正**:在 A 這種 baseline 本來就快(~497µs)的 cell,**warmer 的 preprocessing(含 process 啟動,~250µs 起跳)反而 > first-query 省下的時間 → e2e 比 baseline 還慢**。first-query 改善(layers_5 −30%、2f −79%)是真的,但 **e2e 的勝負要看 baseline 夠不夠慢**:e2e 真正贏是在慢 workload(C:baseline 1058µs → 2e_K10 e2e 462µs = **−56%**)。這推翻舊 P1「A 上 layout+layers_5 e2e −60%」的說法。

![Workload A 上 layout × strategy 的效果](figures/out/02_layout_effect.png)

*圖 2(P0):Workload A、async first-query,各 layout 的 baseline 與策略(2f ≈ −79% first-q)。注意 first-query 與 end-to-end 的勝負不同(見上表)。*
...
*（P0 重跑:clean DB、layout orig、async first-query;A/B 在 N=5 落底、C 需 N=92。3-layout 版見 Figure 11、churned-DB 版見 Figure 12,皆已 P0。）*
...
*（P0:`run_p0_churn.py`——量測走 run_p0(drop-caches+verify+warmer),churn 用 harness write 模式製造;static t=0 hotset 跨 checkpoint 重用。）*
...
*（P0:`run_p0.py --mem-limit 20M`(systemd-run --user --scope)vs master unlimited;比值近 1.0 → RAM 壓力幾乎不影響首查。）*
...
page**。我們用 **prefetch（提前 load）** 把它們先放進 memory。**P0 重跑結論**
(權威數字見 §5 / [overall_results.md P0 master 表](overall_results.md)):first-query
最低是載整個 working set 的 **2f_slru(−79 ~ −90%)**,結構式 **layers_5 / 2e_K10**
...
preprocessing 反而蓋過 first-query 的省時、e2e 不升反降。這些結果在 50k 寫入
churn、cgroup `MemoryMax=20M` 記憶體壓力、與 cadence 重暖三條 robustness 軸下穩定
...
*（P0:Z 的 layers_N sweep,`p0_runs_nsweep_dense/`;hotspot 位置改變但 N=5 elbow 與 plateau 形狀不變 → layers_N 的效益是結構性的。）*
...
*（P0:Figure 11 = dense layers_N × 3 layout(clean,`p0_runs_nsweep_dense/`);Figure 12 = layers_N 在最終 churned DB(50k churn 後,`run_p0_churn.py`)。）*

...
> pipeline。本 paper 所有 §5 / §6 數據均依此 protocol measurement；歷史上使用
> per-file `posix_fadvise` / `sudo drop_caches` 量出的numbers將由 P0 master
...
規劃的 Level 2 才下放到 SSD line / namespace isolation——是 future work。
...
所有experiment shared一個固定的 reference database（由 `testdb_builder.py` 產生），
...
每個 cell 對**同一 hotset** 跑 **pread（oracle）/ async（hint）雙臂**，外加每 (workload,layout) 一個 **baseline（無 prefetch）cell** 當分母。輸出欄位（`run_p0.py` → `raw_p0.csv` / `summary_p0.csv`）：`verify_cold_pct`、`verify_delivery_pct`、`fq_pread`、`fq_async`、`delivery_pct`、`preproc_us`（= `warmer_us`）、`e2e_us`（= preproc + fq）、`avg_us`、`majflt`、`minflt`，每筆並記 `ra_kb` 與當下 `loadavg/memavail`。full schema 見
...
**P0 pipeline重產**(`run_p0.py --regen-hotsets`:P0 全機冷清 → workload warmup → mincore residual快照;
2e 的 top-K leaf 由 workload frequency決定、deterministic),freeze清單 `p0_runs/hotset_freeze.sha256`
...
ck0→ck10 無上升趨勢;三 layout(orig/vacuum/ta)皆然(資料 [`p0_runs_churn/`](p0_runs_churn/))。
...
**DB ~102 MB、RAM 用 cgroup `MemoryMax=20M` 砍到 20 MB**（`run_p0.py --mem-limit 20M`,
systemd-run --user --scope),first query 的improvement**幾乎不受影響**:
...
*（P0:`run_p0.py --mem-limit 20M`(systemd-run --user --scope)vs master unlimited;ratio近 1.0 → RAM pressure幾乎不影響first-query。）*
...
- **Type-aware Physical Segregation (Level 2)**：把 type-aware layout 從
  ...
  layout。在 FEMU SSD emulator 上做。full PoC spec 已寫
  [type_aware_physical_segregation/README.md](type_aware_physical_segregation/README.md)。
...
*（P0:Z 的 layers_N sweep,`p0_runs_nsweep_dense/`;hotspot 位置改變但 N=5 elbow 與 plateau 形狀不變 → layers_N 的效益是structure性的。）*

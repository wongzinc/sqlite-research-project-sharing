...
2. **沒有萬用解**：最適合載幾個 page，跟「資料怎麼排」「query 什麼樣」強烈相關。
...
6. **Memory 吃緊也撐得住**：把 RAM 限到 20 MB，first query 的改善幾乎不受影響。

   ![RAM-pressure heatmap (20 MB cgroup vs unlimited)](figures/out/06_ram_pressure_heatmap.png)

   *圖 6：把可用 RAM 砍到 20 MB（A/B/C × 3 layout × 7 策略 = 63 個 cell）。每 cell 的「20M / 不限」比值**全部落在 0.90–1.19**——memory pressure 幾乎不影響 first query。*
